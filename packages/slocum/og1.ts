...
export interface Og1Field {
  ...
  required: boolean;
  group: 'platform' | 'deployment' | 'people' | 'program' | 'quality' | 'sensor';
...
export const OG1_FIELDS: readonly Og1Field[] = [
  ...
  // ── program ──
  { key: 'site', label: 'Site', required: false, group: 'program' },
  { key: 'siteVocabulary', label: 'Site vocabulary', required: false, group: 'program' },
  { key: 'program', label: 'Program', required: false, group: 'program' },
  { key: 'programVocabulary', label: 'Program vocabulary', required: false, group: 'program' },
  { key: 'project', label: 'Project', required: false, group: 'program' },
  { key: 'network', label: 'Network', required: false, group: 'program' },
  { key: 'dataUrl', label: 'Data URL', required: false, group: 'program' },
  { key: 'doi', label: 'DOI', required: false, group: 'program' },
  { key: 'webLink', label: 'Web link', required: false, group: 'program' },
  { key: 'comment', label: 'Comment', required: false, group: 'program' },
...
/**
 * The glider's own behavior state, when it is logged.
 *
 ...
 */
const BEHAVIOR = ['cc_final_behavior_state', 'm_depth_state'];

/** Slocum `cc_final_behavior_state` → OG1 PHASE, from the published table. */
const BEHAVIOR_TO_PHASE: Record<number, number> = {
  0: 5, // inflection
  1: 2, // descent
  2: 1, // ascent
  3: 4, // parking
 ...
 *
 * Uses the glider's own behavior state where the file carries it. Otherwise
...
export function derivePhase(
  ...
  if (behavior && behavior.values.some(Number.isFinite)) {
    method =
      'From the glider’s own cc_final_behavior_state, via the OceanGliders Slocum ' +
      'phase translation table.';
    let last = PHASE_UNKNOWN;
    for (let i = 0; i < n; i++) {
      const state = behavior.values[i];
      if (Number.isFinite(state)) last = BEHAVIOR_TO_PHASE[state] ?? PHASE_UNKNOWN;
      phase[i] = last;
    }
  } else {
    method =
      'Inferred from the rate of change of pressure: the glider’s own behavior state ' +
    ...
    // Rate by central difference over the nearest finite neighbors, so gaps
...
export function buildOg1(table: Table, metadata: Og1Metadata): Og1Result {
  ...
  const behavior = column(table, BEHAVIOR);
  const { phase, segment, profile, direction, method } = derivePhase(time, pressure, behavior);
