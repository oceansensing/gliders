 ...
 *
 * # Why these tables are here and also in the map package
 *
 * `packages/ocean-map/data/map-palette.json` carries the same twenty, plus
 * five more it built for itself. Those five exist to dodge the map's own
 * feature gamut so a marker stays visible on top of them, which is a problem
 * this plot does not have: nothing sits over anything else here.
 *
 * Importing the map's palette would make the decoder depend on the map, and
 * `packages/ocean-map` is written to be lifted out into another site and a
 * native app — see its BOUNDARIES.md. So the tables are duplicated, and what
 * stops them drifting is a check rather than a promise: `test:slocum`
 * compares every shared name against the map's palette and fails on any
 * difference. That is the bargain the two `REFRESH_HOURS` copies already
 * strike in this repository.
 *
