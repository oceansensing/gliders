...
/**
 * How much one file may decompress to.
 *
 * A block costs three input bytes at minimum — two of length and one of
 * payload — and yields up to 32 KiB, so the format tops out near 10,000:1 and
 * a few megabytes of input can ask for tens of gigabytes of output. The
 * per-block cap does not bound the file; only a running total does.
 *
 * 512 MB is far past any real Slocum file. A recovered `.dbd` is single-digit
 * megabytes and the compressed forms are the ones sent over Iridium, which
 * are smaller still — so anything approaching this is not a glider file.
 */
...
export function decompressStream(bytes: Uint8Array): Uint8Array {
  ...
  while (i + 2 <= bytes.length) {
    ...
    if (total > MAX_STREAM_BYTES) {
      throw new Error(
        `LZ4: expands to more than ${Math.round(MAX_STREAM_BYTES / (1024 * 1024))} MB — not a Slocum file`,
      );
    }
  }
