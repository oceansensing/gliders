...
`colormaps.ts` carries twenty: matplotlib's perceptually uniform set and the
classics, and the eleven from [cmocean](https://matplotlib.org/cmocean/).

```ts
import { COLORMAPS, STANDARD, CMOCEAN, sample } from '@c4po/slocum/colormaps';

sample('cmo.haline', 0.5); // 'rgb(...)', clamped rather than wrapped
```

They are ten-stop samples of the published maps — recognizable, not the exact
tables — interpolated in sRGB. `sample` clamps out-of-range `t` to the end
color rather than wrapping, so a value at a pinned limit takes the end of the
scale instead of reappearing at the other end of it.

**The same twenty are in `packages/ocean-map`**, and are duplicated on
purpose: importing that package's palette would make this one depend on it,
and it is written to be lifted out into another site and a native app.
`npm run test:slocum` compares every shared name against it stop-for-stop, so
the copies cannot drift quietly.
...
| `colormaps.ts` | twenty color scales, for a plot's color axis |
