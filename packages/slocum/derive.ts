 ...
 *
 * **Off by default, and labeled.** Every column here is `source: 'derived'`,
 ...
 * drew sea-ice concentration in the bottom hundredth of its color ramp — and
...
/**
 * Whether a sensor is one the seawater properties are computed *from*.
 *
 * Exported because a caller that narrows the table has to keep these: the
 * derived columns are computed from the table it is handed, so dropping the
 * CTD drops everything derived from it. A page filtering the table by what
 * the reader ticked for export took the CTD out from under the salinity they
 * had ticked, and the columns silently vanished.
 *
 * The family suffix is stripped first, so `sci_water_temp_tbd` counts.
 */
export function isSeawaterInput(name: string): boolean {
  const bare = name.replace(FAMILY_SUFFIX, '');
  return [...CONDUCTIVITY, ...TEMPERATURE, ...PRESSURE].some((n) => n === name || n === bare);
}
